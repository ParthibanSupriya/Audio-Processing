                ...
                **/
                ...
                for (var i = 0; i < totalSamples; ++i) {
					...
					if (frequency < nyquistFrequency)
						sampleValue+= 1.0*Math.sin(2.0 * Math.PI * frequency * currentTime);
            ...
            case "fm": // FM
                ...
                **/
                ...
                for(var i = 0; i < totalSamples; ++i) {
					...
					var multiplier = 1;
					//adsr for the modulator
                    if (useADSR) {
						if(currentTime <= attackDuration/sampleRate)//attack
							multiplier = lerp(0, 1, i/attackDuration);
						else if(currentTime > attackDuration/sampleRate && currentTime <= (attackDuration/sampleRate+decayDuration/sampleRate))//decay
							multiplier = lerp(1, sustainLevel, (i-attackDuration)/decayDuration);
						else if (currentTime > (attackDuration/sampleRate+decayDuration/sampleRate) && currentTime <= duration - releaseDuration/sampleRate)//sustain
							multiplier = sustainLevel;
						else
							multiplier = lerp(0, sustainLevel, (totalSamples-i)/releaseDuration);//release
					}
						
					modulationTerm = multiplier * modulationAmplitude*Math.sin(2.0 * Math.PI *modulationFrequency * currentTime);
					sampleValue = carrierAmplitude*Math.sin(2.0 * Math.PI *carrierFrequency * currentTime + modulationTerm);
            ...
            case "karplus-strong": // Karplus-Strong algorithm
                ...
                **/
                ...
                for(var i = delay + 1; i < totalSamples; ++i) {
					...
					if (Math.random() <= b) //adding blend factor
